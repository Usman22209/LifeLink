...
export const DONOR_QUESTIONS: QuestionnaireQuestion[] = [
  {
    ...
    expectedAnswer: true,
    failureReasonEn: "Minimum weight requirement for blood donation is 50 kg (110 lbs) for your safety.",
    failureReasonUr: "آپ کی حفاظت کے لیے خون دینے کے لیے کم از کم 50 کلوگرام وزن ضروری ہے۔",
  },
  {
    id: "recent_donation",
    ...
    failureReasonEn: "A 90-day recovery interval is required between whole blood donations.",
  ...
  },
  {
    ...
    expectedAnswer: false,
    failureReasonEn: "Active chronic heart, lung, or insulin-dependent diabetes conditions preclude donation.",
    failureReasonUr: "دل، پھیپھڑوں یا انسولین پر منحصر ذیابیطس کے مریض خون نہیں دے سکتے۔",
  },
  {
    id: "recent_infection",
    questionEn: "In the last 28 days, have you had COVID-19, active fever, or severe flu?",
    questionUr: "کیا گزشتہ 28 دنوں میں آپ کو کووڈ-19، بخار یا شدید زکام رہا ہے؟",
    expectedAnswer: false,
    failureReasonEn: "You must be completely symptom-free for at least 28 days following infection.",
    failureReasonUr: "کسی بھی انفیکشن یا بخار کے بعد کم از کم 28 دن صحت مند رہنا ضروری ہے۔",
  },
  {
    id: "blood_infections",
    questionEn: "Have you ever tested positive for Hepatitis B/C, HIV, or Cancer?",
    questionUr: "کیا آپ کا کبھی ہیپاٹائٹس، ایچ آئی وی یا کینسر کا ٹیسٹ مثبت آیا ہے؟",
    expectedAnswer: false,
    failureReasonEn: "Blood safety regulations strictly defer donors with a history of blood-borne diseases.",
    failureReasonUr: "طبی قوانین کے مطابق ہیپاٹائٹس یا خون کے انفیکشن کے حامل افراد خون نہیں دے سکتے۔",
  },
  {
    id: "surgery_tattoo",
    questionEn: "In the last 6 months, have you had major surgery, new tattoo, or piercing?",
    questionUr: "کیا پچھلے 6 ماہ میں آپ کی بڑی سرجری یا نیا ٹیٹو/پیرسنگ ہوئی ہے؟",
    expectedAnswer: false,
    failureReasonEn: "A 6-month safety waiting period applies following surgery, tattooing, or piercing.",
...
export const evaluateDonorEligibility = (
  ...
  if (dob) {
    ...
    if (!isNaN(birthDate.getTime())) {
      ...
      if (age < 17) {
        reasons.push(`You are ${age} years old. Donors must be at least 17 years old.`);
        reasonsUr.push(`آپ کی عمر ${age} سال ہے۔ خون دینے کے لیے کم از کم 17 سال عمر ضروری ہے۔`);
      } else if (age > 65) {
        reasons.push(`You are ${age} years old. Donors must be 65 years or younger.`);
        reasonsUr.push(`آپ کی عمر ${age} سال ہے۔ خون دینے کے لیے زیادہ سے زیادہ 65 سال عمر ہو سکتی ہے۔`);
      }
    }
  }
  ...
  if (answers.recent_donation === true) {
    ...
    const diffDays = Math.max(
      1,
      Math.ceil((nextDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
    );
    reasons.push(
